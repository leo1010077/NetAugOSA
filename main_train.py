...
def parse_option():
    ...
    parser.add_argument('--experiment', type=str, default='Debug', help="experiment name")
    parser.add_argument('--NetAug', type=list[float], default=[1, 2, 4], help="[1] for no Aug")
    # easy config modification
    parser.add_argument('--lr', type=float, default=0.001, help="lr")
    parser.add_argument('--num_epochs', type=int, default=2, help="epoch")
...
def test(model, aug, device, test_loader, fold, result, epoch, wandb_logger=None):
    ...
    with torch.no_grad():
        for batch_idx, (data, target, _C) in enumerate(loader):
            ...
            # =========================================================
            data = data.float()
            data, _C = data.to(device), _C.to(device)
...
def main(args):
    ...
    for fold, (train_ids, test_ids) in enumerate(kfold.split(dataset)):
        ...
        model = NetAugResNet18(base_net=basemodel, ResBlock=BasicBlock_baseV2, layer_list=[2, 2, 2, 2],
                               aug_width_mult_list=[1, 2, 3, 4], n_classes=4)
        model = model.to(device)
        ...
        # =========================================================
        y_test, y_score = statics(basemodel, aug, device, test_loader, best_model_save_path_1x, experiment_name, num_class)
        # =========================================================
        # Draw ------------------------
        # =========================================================
        path_txt = 'resalut/' + experiment_name + '/output.txt'
        path_txt2 = 'resalut/' + experiment_name + '/outputall.txt'
        path_f1 = 'resalut/' + experiment_name + '/f1.txt'
