...
from Model.base.layers import InvertedBlock, SeInvertedBlock, SELayer, ConvLayer_padding
from Model.NetAug.layers import DynamicConvLayer_padding
...
def sort_param(
    ...
    #print(torch.index_select(param.data, dim, sorted_idx).shape)
    param.data.copy_(
...
def sort_norm(norm, sorted_idx: torch.Tensor) -> None:
    #print(norm.weight.shape, len(sorted_idx))
...
def sort_channels_resblock_BN(block) -> None:
    #print(block)
    if isinstance(block, (DynamicConvLayer_padding)):

        importance = torch.sum(torch.abs(block.norm.weight))
        sorted_importance, sorted_idx = torch.sort(importance, dim=0, descending=True)
        # sort based on sorted_idx
        # if block.conv.in_channels == block.conv.out_channels:
        #     sort_param(block.conv.weight, 1, sorted_idx)
        ...
        # else:
        #     sort_param(block.conv.weight, 1, sorted_idx)
        sort_param(block.conv.weight, 0, sorted_idx)
        sort_norm(block.norm, sorted_idx)
    else:
        raise NotImplementedError