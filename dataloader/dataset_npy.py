...
import torch
class OSAnpyDataset(data.Dataset):
    def __init__(self, floader_path, train=True):
        ...
        # if train:
        #     self.datas, self.labels = x_train, y_train
        # else:
        #     self.datas, self.labels = x_test, y_test
    def __len__(self):
    ...
    def __getitem__(self, idx):
        ...
        image = cv2.imdecode(np.fromfile(file_path, dtype=np.uint8), -1)[:, :, :-1]
...
class OSAnpyDataset_3C(data.Dataset):
    def __init__(self, floader_path, train=True):
        self.floader_path = floader_path + '/*/'
        self.labels = []
        self.datas = []
...
if __name__ == "__main__":
    path = 'J:/DATASET/OSA/DataSet/Dataset_newmel_png/train/'
    # dataset = OSAnpyDataset(path)
    dataset = OSAnpyDataset(path)
