...
class OSAnpyDataset_augment_mixup_mix_3d_png(data.Dataset):
    ...
    def CircularPad(self, image):
        long = int(Image.size(dim=2))

        main_event = main_event_detect_tensor(image)
        rate = math.ceil(long / len(main_event)) + 1
        #print(long, len(main_event), rate)
        main_event_Image = Image[:, :, main_event]
        main_event_Image = main_event_Image.repeat(1, 1, rate)
        main_event_Image = main_event_Image[:, :, :long]
        #print(main_event)
        return main_event_Image
