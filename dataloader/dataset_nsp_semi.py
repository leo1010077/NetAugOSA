...
import torch
class OSAnpyDataset_nsp_semi(data.Dataset):
    def __init__(self, floader_path, semi_path=None, train=False, model=None, device=None):
        ...
        if self.train == False:
            with torch.no_grad():
                for semi_path in semi_path_list:
                    ...
                    for mmod in model:
                        output_mmod = mmod(image_semi)
                        output = output + output_mmod
    ...
    def __getitem__(self, idx):
        ...
        if self.train == False:
            semi_idx = random.randint(0, 31443)
