...
from dataloader.augment import SpecAug, jitter, Resort
from sklearn.model_selection import KFold,train_test_split

class OSAnpyDataset_augment_mixup_mix_3d_png(data.Dataset):
    def __init__(self, floader_path, Transform_AUG, train_id=None, train=True, ORI=False, SPLICE=True, MIXUP=True, COVER=True, doMixedFrquencyMasking=True, ReSort=False):
        ...
        if self.ReSort:
            self.resort = Resort()
        self.doMixedFrquencyMasking = doMixedFrquencyMasking
        self.AUGS = self.Aug_manage()
    ...
    def __getitem__(self, idx):
        ...
        if self.train:
            if self.ReSort:
                image = self.resort(image)
            choose_AUG = random.choice(self.AUGS)
            if choose_AUG == 0:
                image = image
            elif choose_AUG == 1:
                ...
                image, label = self.MixedFrquencyMaskingAUG(image, file_path, _c)
    ...
    def Aug_manage(self):
        ...
        if self.COVER == True:
            augs.append(3)
        if self.doMixedFrquencyMasking == True:
            augs.append(4)
        return augs
    ...
    def MixedFrquencyMaskingAUG(self, image, file_path, _c):
        sub_file, sub_label = self.list_woself_returnC(file_path, _c)
        sub_image = self.transform_start(Image.open(sub_file))
        image_re, label_re = self.MixedFrquencyMasking(image, sub_image, _c, sub_label)
        return image_re, label_re
    ...
    def MixedFrquencyMasking(self, main_mel, sub_mel, main_c, sub_c):
        out_mel = main_mel
        hight = main_mel.shape[1]
        range_max = hight // 2
        range_min = hight // 4
        range = random.randint(range_min, range_max)
        start = random.randint(0, hight - range)
        out_mel[:, start:start + range, :] = sub_mel.resize_(3, range, 690)
        if sub_c != 3:
            rate = range // hight
            out_C = np.eye(4)[main_c] * (1 - rate) + np.eye(4)[sub_c] * (rate)
        else:
            out_C = np.eye(4)[main_c]
