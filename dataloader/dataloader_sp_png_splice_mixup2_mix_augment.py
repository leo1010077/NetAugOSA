...
class OSAnpyDataset_augment_mixup_mix_3d_png(data.Dataset):
    def __init__(self, floader_path, train_id=None, train=True, aug_type='mix', doaugment_splice=True, doaugment_mixup=True, doaugment_cover=True, doaugment_contrast=True, splice_chance=0.95):
    ...
    def __getitem__(self, idx):
        ...
        main_C = np.eye(4)[labels]
        image = cv2.imread(file_path)
        if self.train:
            if self.aug_type == 'mix':
                if labels != 3:
                    if self.doaugment_splice:
                        sub_file = self.listwithout_self_3(file_path, labels)
                    ...
                    if self.doaugment_mixup:
                        sub_file, sub_C = self.list_woself_returnC(file_path, labels)
                        sub_mel = cv2.imread(sub_file)
                        image, labels = self.mixup_label(main_mel=image, main_c=main_C, sub_mel=sub_mel, sub_c=sub_C)
                elif labels == 3 and self.doaugment_mixup:
                    labels = main_C
    ...
    def listwithout_self_3(self, main_file, _class):
        #print(self.train_id)
    ...
    def mixup_label(self, main_mel, sub_mel, main_c, sub_c):
        ...
        if sub_c != 3:
            out_C = main_c * rate + np.eye(4)[sub_c] * (1 - rate)
        else:
            out_C = main_c
        #print(len(out_C))
