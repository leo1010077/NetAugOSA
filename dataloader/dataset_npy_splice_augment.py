...
import math
from dataloader.utils import main_event_detect

class OSAnpyDataset_augment_3d(data.Dataset):
    def __init__(self, floader_path, train_id=None, train=True, doaugment_splice=True):
    ...
    def __getitem__(self, idx):
        ...
        if self.train:
            if self.doaugment_splice and labels != 3:
                sub_file = self.listwithout_self_2(file_path, labels)
                sub_mel = np.load(sub_file)
                image = self.splice_2_mel(main_mel=image, sub_mel=sub_mel, chance=0.95)
    ...
    def back(self, main_mel, sub_mel, main_mel_indxex, sub_mel_index, OK_main_index_back):
        ...
        if start + len(sub_mel_index) > 345:  # 如果後面的空間不構放入新的聲音事件就往前移
            not_enough = start + len(sub_mel_index) - 345 - 1  # 算出不夠的量
            main_mel[:, :345 - not_enough] = main_mel[:, not_enough:]  # 往前移
