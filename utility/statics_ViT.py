...
def statics(model, device, test_loader, model_save_path, exp, num_class):
    ...
    with torch.no_grad():
        for inputs, labels, _C in tqdm(test_loader):
            data, target = inputs.to(device).float(), _C.to(device)
            output = model(data)
            ...
            lbllist = torch.cat([lbllist, _C.view(-1).cpu()])
        y_prob = np.concatenate(y_prob)

    pathdir = 'result/' + exp
    path = 'result/' + exp + '/outputall.txt'
    path_out = 'result/' + exp + '/output.txt'
...
def calc_NetAug(model, device, test_loader,num_class): #計算spesicifity sensitivity
    # =========================================================
