...
def parse_option():
    ...
    parser.add_argument('--experiment', type=str, default='0415_resnet18qq_nofinetune_re', help="experiment name")
    parser.add_argument('--init_way', type=str, default='kaiming_uniform', help="kaiming_uniform or None")
    parser.add_argument('--finetune_mode', type=str, default='no', help="kaiming_uniform or None")
...
def finetune_test(model, device, test_loader, fold, result, epoch, wandb_logger=None):
    ...
    with torch.no_grad():
        for batch_idx, (data, target, _C) in enumerate(loader):
            data = data.float()
            data, target, _C = data.to(device), target.to(device), _C.to(device)
            for lab in _C.detach().cpu().numpy():
                gt_labels.append(lab)
            output = model(data)
            loss = criterion(output, target)
